...
@js.native
class ANTLRInputStream(input: Object) extends js.Object

@js.native
class FIRRTLLexer(input: ANTLRInputStream) extends js.Object

@js.native
class CommonTokenStream(lexer: FIRRTLLexer) extends js.Object

object PredictionMode {
  val SLL = 0
  val  LL = 1
}

@js.native
trait ParserATNSimulator extends js.Object {
  def setPredictionMode(mode: Int): Unit
}
...
@js.native
trait ParserRuleContext extends js.Object {
  def getStart: String
}

object FIRRTLParser {
  type CircuitContext = js.Object
  @js.native
  trait InfoContext extends js.Object {
    def getText: String
  }
}

@js.native
class FIRRTLParser(input: CommonTokenStream) extends js.Object {
  val _interp: ParserATNSimulator = ???
  def getInterpreter = _interp
  def getNumberOfSyntaxErrors: Int = ???
  def circuit: FIRRTLParser.CircuitContext = ???
}
  ...
  def parse(lines: Iterator[String], infoMode: InfoMode = UseInfo): Circuit = {
    ...
    val (parseTimeMillis, cst) = time {
      val parser = {
        ...
        val inStream = lines.map{s => new ByteArrayInputStream((s + "\n").getBytes("UTF-8")) }
