...
class YamlFileReader(file: String) {
  ...
  def parse[A]: Seq[A] = {
    if (new File(file).exists) {
      val yamlString = scala.io.Source.fromFile(file).toString // .getLines.mkString("\n")
      YAML.parse(yamlString) match {
        case a: Seq[A] => a
        case _    => throw new Exception("Not an a")
      }
    }
